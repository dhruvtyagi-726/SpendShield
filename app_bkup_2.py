    ...
    for _, row in df[df["type"] == "debit"].iterrows():
        ...
        if key in seen:
            alerts.append({
                ...
                "severity": "High",
                "message": f"Duplicate payment found: {row['description']} Rs.{row['amount']} via {row['payment_method']} on {row['date']}",
            })
    ...
    for _, row in debit_df.iterrows():
        if row["amount"] >= threshold:
            alerts.append({
                ...
                "severity": "Medium",
                "message": f"Review high-value transaction: {row['description']} Rs.{row['amount']} on {row['date']}",
            })
        ...
        if str(row["category"]).lower() == "other":
            alerts.append({
                ...
                "severity": "Medium",
                "message": f"Unknown category transaction: {row['description']} Rs.{row['amount']} on {row['date']}",
            })
    ...
    for _, row in repeated.iterrows():
        if row["count"] >= 2:
            alerts.append({
                ...
                "severity": "Low",
                "message": f"Recurring subscription detected: {row['description']} Rs.{row['amount']}",
            })
